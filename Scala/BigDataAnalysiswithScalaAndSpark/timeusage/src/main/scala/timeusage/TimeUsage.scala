...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val TypedLine = line.head.toString::line.tail.map(stingVal=> stingVal.toDouble)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val PrimaryNeeds_Prefix =  List("t01","t03", "t11","t1801","t1803")
    val WorkingActivities_Prefix = List("t05","t1805")
    val OtherActivities_Prefix = List("t02", "t04", "t06", "t07", "t08", "t09",
                                "t10", "t12", "t13", "t14", "t15", "t16","t18")


    def ActivityTypeFilter(col_name: String, ActivityTypes: List[String]) : Boolean = {
      ActivityTypes.map( prefix => col_name.startsWith(prefix) ).reduce((a,b) => a||b)
    }

    val PrimaryNeeds = columnNames.filter(col_name => ActivityTypeFilter(col_name,PrimaryNeeds_Prefix))
                                  .map(col_name => col(col_name))

    val WorkingActivities = columnNames.filter(col_name => ActivityTypeFilter(col_name,WorkingActivities_Prefix))
                                       .map(col_name => col(col_name))

    val OtherActivities = columnNames.filter(col_name => ActivityTypeFilter(col_name,OtherActivities_Prefix))
                                     .filterNot(col_name => ActivityTypeFilter(col_name,PrimaryNeeds_Prefix))
                                     .filterNot(col_name => ActivityTypeFilter(col_name,WorkingActivities_Prefix))
                                     .map(col_name => col(col_name))
    // ???
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when( col("tesex") === 1 , "male" )
                                ...
                                .as("sex")
    //???
    val ageProjection: Column = when(col("teage") >=15 && col(colName = "teage")<=22,"young")
                                .when(col("teage") >=23 && col(colName = "teage")<=55,"active")
                                .otherwise("elder")
                                .as("age")
      //???
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.select(
      ...
      col("other")
          ...
          .agg(
            round(avg(columnName = "primaryNeeds").as("primaryNeeds"),1),
            round(avg(columnName = "work").as("work"),1),
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String = {
    ...
    ("SELECT working, sex, age, ROUND(AVG(primaryNeeds), 1) AS primaryNeeds," +
      ...
      " FROM  %s GROUP BY working, sex, age ORDER BY working, sex, age").format(viewName)
  }
    //???
